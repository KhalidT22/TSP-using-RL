...
class DeliveryQAgent(object):
    ...
    def __init__(self,xy,max_box,method,boundary_index,boundary_points,mydict,labels,n_cluster,simplices,vertices,states_size,actions_size,
    alpha = 1,beta=1,gamma = 0.9,lr = 0.6):
        self.xy = xy
        self.method = method
        self.boundary_index = boundary_index
    ...
    def plotcurrenttour(self,xy,list1,n,simplices,vertices,labels):
        ...
        for i in tour:
            if i == tour[-1]:
                k = 'city'+str(i+1)
                g = 'city'+str(tour[0]+1)
                plt.arrow(coordinates[k][0],coordinates[k][1],diff(coordinates[k],coordinates[g])[0],diff(coordinates[k],coordinates[g])[1], head_width=0.15)
    ...
    def chosen_point(self,x, point_list):
        ...
        for point in point_list:
            distance = math.sqrt((self.xy[point][0] - self.xy[x][0])**2 + (self.xy[point][1] - self.xy[x][1])**2) # calculate the Euclidean distance between x and the current point
            weight = 1/distance # smaller distance leads to higher weight
            weights.append(weight)
        total_weight = sum(weights) # calculate the sum of all weights
        probabilities = [weight/total_weight for weight in weights] # normalize the weights to obtain probabilities
        ...
        if self.method == "cluster":
                ...
                if self.end == 1:
                    ...
                    if s in self.boundary_index:
                        ...
                        if len(bound) == 1 and len(inter) >0:
                            ...
                            k = [i for i in range(self.states_size) if i not in self.states_memory]
                            l = [self.labels[i] for i in k]
                            #print("the labs of un is ",l)
                            if len(lis) == 0:
                                k = [i for i in range(self.states_size) if i not in self.states_memory]
                                self.plotcurrenttour(self.xy,self.states_memory,self.actions_size,self.method,self.simplices,self.vertices,self.labels)
                                k= 0
                                #print(self.Q_5)
                                #print(self.Q_100)
            ...
            if self.i == self.num -1:
                ...
                if np.random.rand() > self.epsilon:
                    ...
                    lis = [i for i in range(self.states_size) if i not in self.states_track]
                ...
                if np.random.rand() > self.epsilon:
                    ...
                    for i in range(self.actions_size):
                        if i in self.states_track:
                            continue
                        q[i] = -q[i] +  self.c*math.sqrt(math.log(self.timestep)/(self.visits[(s,i)]+1))
                    #print(q)    
                    a = np.argmax(q)
        elif self.method == "nn":
            q = np.copy(self.Q2[s,:])
...
def plottour(x,y,list1,n,r,method,simplices,vertices,labels):
    ...
    for i in range(len(set(labels))):
        ...
        plt.plot(points[vertices[c],0],points[vertices[c],1],'o', mec='r', color='none', lw=1, markersize=10)
        c +=1 
...
def run_n_episodes(env,agent,plot1,name="training.gif",n_episodes=1000,render_each=10,fps=10):
    ...
    '''
    if plot1:
        plottour(env.x,env.y, env.stops, env.n_stops, episode_reward,agent.method,agent.simplices,agent.vertices,agent.labels)
    check = 0
    return env,agent,episode_reward
...
def plottour2(x,y,tour,coordinates,obj,simplices,vertices,labels):
    ...
    for simplex in simplices:
        ...
        plt.plot(points[vertices,0],points[vertices,1],'o', mec='r', color='none', lw=1, markersize=10)
    count = 1
    for i in tour:
        if i == tour[-1]:
            k = str(i)
    ...
    for n_stops in nodes:
       ...
       #calc distance btwn points 
        ...
        for k in range(rep):
            ...
            check = {}
            print("################ NODES =  "+str(n_stops)+" STEP "+str(k+1)+" #########################")
            ...
            dist = {(c1, c2): distance(coordinates, c1, c2) for c1, c2 in combinations(cities, 2)}
            ...
            kn = KneeLocator(list(data.keys()), list(data.values()) ,curve='convex', direction='decreasing')
            print(kn)
            k = kn.knee
            if k == None:
                k =3
            ...
            mydict = {i: list(np.where(kmeans.labels_ == i)[0]) for i in range(kmeans.n_clusters)}
            ...
            for i in tour:
                if i == tour[-1]:
                    k = str(i)
                    g = str(tour[0])
                    plt.arrow(coordinates[k][0],coordinates[k][1],diff(coordinates[k],coordinates[g])[0],diff(coordinates[k],coordinates[g])[1], head_width=0.05)
                    plt.text(coordinates[k][0],coordinates[k][1],s = str(count))
            ...
            xy = X
            fixed = True
            env = Delivery(xy,boundary_index,n_stops = n_stops,max_box = max_box,fixed=fixed)
            plot1 = True
            ...
            for method in methods:
                agent = DeliveryQAgent(xy,max_box,method,boundary_index,boundary_points,mydict,labels,n_cluster,simplices,vertices,env.observation_space,env.action_space)
                env,agent,episode_reward = run_n_episodes(env,agent,plot1,"training_"+str(env.n_stops)+"_stops.gif",n_episodes)
